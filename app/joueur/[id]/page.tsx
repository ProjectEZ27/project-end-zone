...
export default async function PublicProfile({ params }: { params: Promise<{ id: string }> }) {
  ...
  let totalJoueursClassement = 0
  let mesPoints = 0
  if (saison) {
    const classementSaison = await calculerClassementSaison(supabase, saison.id)
    const monIndex = classementSaison.findIndex((j) => j.utilisateur_id === id)
  ...
  let matchsAPronostiquer = 0
  if (semaineActuelleData) {
    ...
    if (matchsSemaineActuelle && matchsSemaineActuelle.length > 0) {
      ...
      matchsAPronostiquer = idsMatchsSemaine.filter((mid) => !idsDejaFaits.has(mid)).length
    }
  }
  ...
  return (
    <div style={{ position: 'relative', minHeight: '100dvh' }}>
      ...
      <div style={{ maxWidth: 500, margin: '0 auto', padding: '24px 24px 100px', textAlign: 'center', color: 'white' }}>
        <UserAvatar avatarId={profile.avatar_id ?? 1} size={110} userName={profile.pseudo} />
        <h1 style={{ fontSize: 24, fontWeight: 700, marginTop: 8 }}>{profile.pseudo}</h1>
        ...
        <div style={{ marginTop: 16, textAlign: 'left' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
            ...
            <span style={{ fontSize: 14, fontWeight: 700 }}>Sa saison {saison?.nom ?? ''}</span>
          </div>
            ...
            {resultatsParSemaine.length > 0 && (
              <>
                <div style={{ fontSize: 11, color: '#9fb0c9', marginBottom: 8 }}>
                  Évolution de son pourcentage de réussite
                </div>
                {(() => {
